...
Tick()
{
    ...
    if (bountyStateChanged)
    {
        ...
        {
            Case "Unknown":
                MsgBox, , , from %prevBountyState% to %bountyState% now ??? took %ElapsedTime%
            Case "None":
                MsgBox, , , from %prevBountyState% to %bountyState% now medalDunk took %ElapsedTime%
                ; newScript := medalDunk
            Case "Held":
                MsgBox, , , from %prevBountyState% to %bountyState% now medalFarm took %ElapsedTime%
                ; newScript := medalFarm
                ; SwitchTo(newScript)
                ; handledChange := true
            Case "Progress":
                MsgBox, , , from %prevBountyState% to %bountyState% now Nothing took %ElapsedTime%
                ; newScript := medalFarm
            Case "Complete":
                MsgBox, , , from %prevBountyState% to %bountyState% now medalDunk took %ElapsedTime%
                ; newScript := medalDunk
